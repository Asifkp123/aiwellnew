...
class SplashViewModel {
  ...
  Future<void> handleLetsGetStartedNavigation(BuildContext context) async {
    ...
    final accessTokenExpiryResult = await getAccessTokenExpiryUseCase.execute();
    ...
    bool isAccessTokenValid = accessToken != null && accessToken!.isNotEmpty && accessTokenExpiry != null && accessTokenExpiry! > now;
    print(accessToken);
    ...
    if (!isAccessTokenValid) {
      ...
      final refreshTokenExpiryResult = await getRefreshTokenExpiryUseCase.execute();

      String? refreshToken;
      int? refreshTokenExpiry;
      refreshTokenResult.fold((l) => null, (r) => refreshToken = r);
      refreshTokenExpiryResult.fold((l) => null, (r) => refreshTokenExpiry = r);
        ...
        refreshResult.fold(
          (failure) {
            ...
            Navigator.pushReplacementNamed(
              ...
              arguments: viewModels[SigninSignupScreen.routeName],
            );
            return;
          },
          (success) async {
            ...
            final newAccessTokenExpiryResult = await getAccessTokenExpiryUseCase.execute();
            final newRefreshTokenResult = await getRefreshTokenUseCase.execute();
            final newRefreshTokenExpiryResult = await getRefreshTokenExpiryUseCase.execute();
            ...
            newAccessTokenResult.fold((l) => null, (r) => newAccessToken = r.$1);
            newAccessTokenExpiryResult.fold((l) => null, (r) => newAccessTokenExpiry = r);
            newRefreshTokenResult.fold((l) => null, (r) => newRefreshToken = r);
            newRefreshTokenExpiryResult.fold((l) => null, (r) => newRefreshTokenExpiry = r);
            ...
            isAccessTokenValid = accessToken != null && accessToken!.isNotEmpty && accessTokenExpiry != null && accessTokenExpiry! > now;
    ...
    if (appState is HomeState && isAccessTokenValid) {
      ...
      Navigator.pushReplacementNamed(
        ...
        arguments: viewModels[ProfileScreen.routeName],
      );
    } else {
